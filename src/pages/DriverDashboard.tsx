...
import { useDriverShift, useMyProfile, useAwardPoints } from '@/hooks/useUsers';
...
export default function DriverDashboard() {
  ...
  const handleCollectSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Award 10 points to the driver for collecting waste
      await awardPoints.mutateAsync({ userId: user!.uid, pointsToAdd: 10 });

      setSelectedReports(prev => prev.filter(r => r.id !== collectReportId));
      setCollectReportId(null);
      toast({ title: 'Collected! +10 Points', description: 'Report marked as collected. You earned 10 impact points!' });
    } catch (err: any) {
      toast({ title: 'Error', description: err.message, variant: 'destructive' });
    }
  };
