    ...
    reader.onloadend = () => {
      ...
      img.onload = async () => {
        try {
          ...
          // Generate more predictions (top 15 instead of default 3) to pick up subtle waste elements
          const predictions = await cachedModel.classify(img, 15);
          console.log("Image predictions:", predictions);

          // Tightly scoped waste-related keywords. Added generic terms for bags and construction waste.
          const trashKeywords = [
            'trash', 'garbage', 'waste', 'litter', 'rubbish', 'dumpster',
            'dustbin', 'refuse', 'debris', 'junk', 'scrap', 'landfill',
            'plastic bag', 'carton', 'bottle', 'tire', 'tyre', 'cardboard',
            'styrofoam', 'crate', 'bucket', 'can', 'bin', 'box',
            'bag', 'sack', 'brick', 'tile', 'stone', 'rubble', 'pack', 'pouch'
          ];

          // Threshold down to 1% because MobileNet isn't a dedicated waste model
          const isTrash = predictions.some(p =>
            p.probability > 0.01 &&
            trashKeywords.some(keyword => new RegExp(`\\b${keyword}\\b`, 'i').test(p.className))
          );
